...
// THEME
// ============================================================
function initTheme(){ applyTheme(localStorage.getItem('jg_theme')||'dark'); }
...
function renderDashboard(){
  ...
  document.getElementById('dashStats').innerHTML=`
    <div class="stat"><div class="num">${j.length}</div><div class="lbl">Pertemuan semester ini</div></div>
    <div class="stat"><div class="num">${pct}%</div><div class="lbl">Rata-rata kehadiran</div></div>
    <div class="stat"><div class="num">${g.length}</div><div class="lbl">Entri nilai</div></div>
    <div class="stat"><div class="num" style="color:${oi>0?'var(--bad)':'var(--good)'}">${oi}</div><div class="lbl">Kejadian belum selesai</div></div>`;
...
// ============================================================
function xlsxStyle(wb, ws, opts={}){
  // SheetJS free tier doesn't support cell styles natively.
  // We use conditional formatting via cell comments and named styles
  // Workaround: embed styling via HTML table → convert (supported by XLSX)
  // We generate a styled HTML string and convert it.
  // This function instead returns ws with !sheetFormat set for col widths etc.
...
// Build styled XLSX via HTML table (supports bg color, bold, borders)
function buildStyledSheet(headers1, headers2=null, rows, title, colWidths){
  // headers1: array of {text, colspan, bg}
  // headers2: array of {text} (optional second header row)
  // rows: array of array of {v: value, bold: bool, color: str, bg: str, align: str}
  let html=`<table style="font-family:Arial,sans-serif;font-size:11px;border-collapse:collapse;">`;
  // Title row
  const totalCols=headers1.reduce((s,h)=>s+(h.colspan||1),0);
  html+=`<tr><td colspan="${totalCols}" style="background:#1a3a5c;color:#ffffff;font-size:13px;font-weight:bold;padding:8px 10px;border:1px solid #ccc;">${title}</td></tr>`;
  html+=`<tr><td colspan="${totalCols}" style="padding:4px;border:none;"></td></tr>`;
  // Header row 1
  html+=`<tr>`;
  headers1.forEach(h=>{
    const bg=h.bg||'#1a3a5c';
    const span=h.colspan>1?` colspan="${h.colspan}"`:'';
    const rs=h.rowspan>1?` rowspan="${h.rowspan}"`:'';
    html+=`<td${span}${rs} style="background:${bg};color:#ffffff;font-weight:bold;padding:6px 8px;border:1px solid #aaa;text-align:center;white-space:nowrap;">${h.text}</td>`;
  });
  html+=`</tr>`;
  ...
  if(headers2){
    html+=`<tr>`;
    headers2.forEach(h=>{
      const bg=h.bg||'#2d5fa8';
      html+=`<td style="background:${bg};color:#ffffff;font-weight:bold;padding:5px 8px;border:1px solid #aaa;text-align:center;font-size:10px;white-space:nowrap;">${h.text}</td>`;
    });
    html+=`</tr>`;
  }
  // Data rows
  rows.forEach((row,ri)=>{
    const rowBg=ri%2===0?'#ffffff':'#f0f4fb';
    html+=`<tr>`;
    row.forEach(cell=>{
      const v=cell==null?'':typeof cell==='object'?cell.v:cell;
      const bold=typeof cell==='object'&&cell.bold?'font-weight:bold;':'';
      const color=typeof cell==='object'&&cell.color?`color:${cell.color};`:'';
      const bg=typeof cell==='object'&&cell.bg?`background:${cell.bg};`:`background:${rowBg};`;
      const align=typeof cell==='object'&&cell.align?`text-align:${cell.align};`:'';
      html+=`<td style="${bg}${bold}${color}${align}padding:5px 8px;border:1px solid #dde;white-space:nowrap;">${v===''?'&nbsp;':String(v).replace(/&/g,'&amp;').replace(/</g,'&lt;')}</td>`;
    });
    html+=`</tr>`;
  });
  html+=`</table>`;
...
async function exportJurnalXLSX(){
  ...
  const title=`Jurnal Mengajar — ${currentTeacher.nama} — ${settings.semester_aktif} ${settings.tahun_ajaran_aktif}`;
  const headers1=[
    {text:'Tanggal',rowspan:1},{text:'Kelas',rowspan:1},{text:'Jam Ke',rowspan:1},
    {text:'Nomor TP',rowspan:1},{text:'Deskripsi Tujuan Pembelajaran',rowspan:1},
    {text:'Metode',rowspan:1},{text:'Materi',rowspan:1},{text:'Kendala',rowspan:1},{text:'Catatan',rowspan:1},
    {text:'H',rowspan:1,bg:'#1a6640'},{text:'S',rowspan:1,bg:'#7a5c00'},{text:'I',rowspan:1,bg:'#1a3a7a'},{text:'A',rowspan:1,bg:'#7a1a1a'},
    {text:'Nama Tidak Hadir',rowspan:1}
  ];
  const rows=[];
  list.forEach(j=>{
    const attForDay=periodFilter(attendanceRecords).filter(a=>a.tanggal===j.tanggal&&a.kelas===j.kelas);
    ...
    // Ambil deskripsi TP lengkap
    const tpObj=j.kd?learningObjectives.find(t=>t.id===j.kd):null;
    const tpNomor=tpObj?tpObj.nomor_tp:'-';
    const tpDeskripsi=tpObj?tpObj.deskripsi:'-';
    rows.push([
      {v:j.tanggal,bold:false},
      j.kelas, j.jam_ke||'-',
      tpNomor, tpDeskripsi,
      j.metode||'-', j.materi, j.kendala||'-', j.catatan||'-',
      {v:hadir,bold:true,color:'#1a6640'},{v:sakit.length||'',color:'#7a5c00'},{v:izin.length||'',color:'#1a3a7a'},{v:alpha.length||0,color:alpha.length?'#c0392b':''},
      notHadir||'-'
    ]);
  });
  const ws=buildStyledSheet(headers1, null, rows, title,
    [{wch:12},{wch:6},{wch:7},{wch:10},{wch:45},{wch:20},{wch:40},{wch:25},{wch:20},{wch:5},{wch:5},{wch:5},{wch:5},{wch:40}]
  );
  XLSX.utils.book_append_sheet(wb,ws,'Jurnal');
  XLSX.writeFile(wb,`Jurnal_${settings.semester_aktif}_${settings.tahun_ajaran_aktif}_${today()}.xlsx`);
...
async function exportKeaktifanXLSX(){
  ...
  const dates=[...new Set(periodFilter(participations).filter(p=>studentsByClass(kelas).some(s=>s.id===p.student_id)).map(p=>p.tanggal))].sort();
  const wb=XLSX.utils.book_new();
  const title=`Rekap Keaktifan Kelas ${kelas} — ${settings.semester_aktif} ${settings.tahun_ajaran_aktif}`;
  const headers1=[
    {text:'No'},{text:'Nama Siswa'},
    ...dates.map(d=>({text:fmtDateShort(d)})),
    {text:'Total Skor',bg:'#1a5c3a'},{text:'Rata-rata',bg:'#1a5c3a'}
  ];
  const rows=list.map(s=>{
    const row=[{v:s.no_urut,align:'center'}, s.nama];
    let total=0,count=0;
    dates.forEach(d=>{
      ...
      row.push(skor>0?{v:skor,align:'center'}:{v:'',align:'center'});
    });
    row.push({v:total,bold:true,align:'center',color:'#1a5c3a'});
    row.push({v:count?Math.round(total/count):'',bold:true,align:'center',color:count&&Math.round(total/count)>=70?'#1a5c3a':'#c0392b'});
    return row;
  });
  const ws=buildStyledSheet(headers1, null, rows, title,
    [{wch:5},{wch:28},...dates.map(()=>({wch:10})),{wch:11},{wch:11}]
  );
  XLSX.utils.book_append_sheet(wb,ws,`Keaktifan ${kelas}`);
  XLSX.writeFile(wb,`Keaktifan_${kelas}_${settings.semester_aktif}_${today()}.xlsx`);
...
async function exportKejadianXLSX(){
  ...
  const wb=XLSX.utils.book_new();
  const title=`Catatan Kejadian — ${currentTeacher.nama} — ${settings.semester_aktif} ${settings.tahun_ajaran_aktif}`;
  const headers1=[
    {text:'Tanggal'},{text:'Nama Siswa'},{text:'Kelas'},
    {text:'Jenis'},{text:'Tingkat'},{text:'Deskripsi'},{text:'Tindak Lanjut'},{text:'Status'}
  ];
  const tingkatColor={Ringan:'#1a6640',Sedang:'#7a5c00',Berat:'#7a1a1a'};
  const statusColor={'Open':'#7a1a1a','Closed':'#1a6640'};
  const rows=list.map(i=>{
    const s=studentObj(i.student_id);
    return [
      i.tanggal, s?s.nama:'?', s?s.kelas:'?',
      i.jenis,
      {v:i.tingkat,color:tingkatColor[i.tingkat]||'#333',bold:true},
      i.deskripsi, i.tindak_lanjut||'-',
      {v:i.status==='Open'?'Terbuka':'Selesai',color:statusColor[i.status]||'#333',bold:true}
    ];
  });
  const ws=buildStyledSheet(headers1, null, rows, title,
    [{wch:12},{wch:28},{wch:7},{wch:12},{wch:9},{wch:45},{wch:35},{wch:10}]
  );
  XLSX.utils.book_append_sheet(wb,ws,'Kejadian');
  XLSX.writeFile(wb,`Kejadian_${settings.semester_aktif}_${today()}.xlsx`);
...
function renderGradeInputTable(gtId,namaPenilaian){
  const kelas=document.getElementById('gKelas').value;
  document.querySelector('#gradeInputTable tbody').innerHTML=studentsByClass(kelas).map(s=>{
    let existing='';
    ...
    return `<tr><td class="mono">${s.no_urut}</td><td>${esc(s.nama)}</td>
      <td><input type="number" min="0" max="100" step="0.01" data-student="${s.id}" class="grade-input" placeholder="0-100" value="${existing}"></td></tr>`;
  }).join('');
}
...
async function exportGradesXLSX(){
  ...
  const typeGroups={};
  typeColumns.forEach(c=>{ if(!typeGroups[c.typeName]) typeGroups[c.typeName]=[]; typeGroups[c.typeName].push(c); });
  // Build styled version with 2-row header and colored cells
  ...
  const headers1=[{text:'No',rowspan:2},{text:'Nama Siswa',rowspan:2}];
  typeGroupsArr.forEach(([tn,cols])=>{
    headers1.push({text:`${tn} (${cols[0].bobot}%)`,colspan:cols.length,bg:'#1a3a5c'});
  });
  headers1.push({text:'Nilai Akhir',rowspan:2,bg:'#1a5c3a'});
  const headers2=[];
  typeColumns.forEach(c=>{
    headers2.push({text:c.namaPenilaian,bg:'#2d5fa8'});
  });
  const styledRows=list.map(s=>{
    const avgPT={};
    types.forEach(t=>{
    ...
    let finalScore=0,hasAny=false;
    types.forEach(t=>{ if(avgPT[t.id]!==null){ hasAny=true; finalScore+=avgPT[t.id]*(t.bobot/100); } });
    const row=[{v:s.no_urut,align:'center'},{v:s.nama}];
    typeColumns.forEach(c=>{
      if(c.isKeaktifan){
        const v=avgPT[c.typeId];
        row.push({v:v!==null?v:'',align:'center',color:v!==null&&v<settings.kkm?'#c0392b':''});
      } else if(c.isEmpty){
        row.push({v:'-',align:'center'});
      } else {
        const g=pg.find(x=>x.student_id===s.id&&x.grade_type_id===c.typeId&&x.nama_penilaian===c.namaPenilaian);
        const val=g?parseFloat(g.nilai.toFixed(1)):'';
        row.push({v:val,align:'center',color:g&&g.nilai<settings.kkm?'#c0392b':'',bg:g&&g.nilai>=settings.kkm?'#eafaf1':g&&g.nilai<settings.kkm?'#fdf0ef':undefined});
      }
    });
    const naVal=hasAny?parseFloat(finalScore.toFixed(1)):'';
    row.push({v:naVal,bold:true,align:'center',color:hasAny&&finalScore<settings.kkm?'#c0392b':'#1a5c3a'});
    return row;
  });
  const ws2=buildStyledSheet(headers1, headers2, styledRows, title,
    [{wch:5},{wch:28},...typeColumns.map(()=>({wch:13})),{wch:13}]
  );
  XLSX.utils.book_append_sheet(wb,ws2,`Nilai ${kelas}`);
  XLSX.writeFile(wb,`Nilai_${kelas}_${settings.semester_aktif}_${today()}.xlsx`);
...
async function backupAllData(){
  ...
  try{
    ...
    const backup={meta:{exported_at:new Date().toISOString(),app:'Jurnal Guru Digital',version:'3.0',teacher:currentTeacher.nama,semester:settings.semester_aktif,tahun_ajaran:settings.tahun_ajaran_aktif},
      students:s.data||[],journal_entries:j.data||[],attendance:a.data||[],incidents:inc.data||[],grade_types:gt.data||[],grades:g.data||[],participation:p.data||[],learning_objectives:lo.data||[]};
...
async function exportSiswaXLSX(){
  ...
  if(!list.length){ showToast('Tidak ada data','error'); return; }
  const wb=XLSX.utils.book_new();
  const title=`Daftar Siswa${kelas?' Kelas '+kelas:''} — ${currentTeacher.nama}`;
  const headers1=[{text:'No'},{text:'No. Urut'},{text:'Nama Siswa'},{text:'Kelas'},{text:'Jenis Kelamin'},{text:'NISN'}];
  const rows=list.map((s,i)=>[
    {v:i+1,align:'center'},{v:s.no_urut,align:'center'},
    {v:s.nama,bold:true},
    {v:s.kelas,align:'center'},
    {v:s.jk==='L'?'Laki-laki':'Perempuan'},
    {v:s.nisn||'-',align:'center'}
  ]);
  const ws=buildStyledSheet(headers1, null, rows, title,
    [{wch:5},{wch:9},{wch:32},{wch:10},{wch:14},{wch:14}]
  );
  const kelasArr=kelas?[kelas]:getKelasList();
  XLSX.utils.book_append_sheet(wb,ws,kelas?`Kelas ${kelas}`:'Semua Siswa');
  XLSX.writeFile(wb,`DaftarSiswa${kelas?'_'+kelas:''}_${today()}.xlsx`);
