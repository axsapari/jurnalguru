...
function renderDashboard(){
  ...
  kelasCards.innerHTML=['9A','9B','9C'].map(kelas=>buildKelasCard(kelas,a,p)).join('');
...
async function exportJurnalXLSX(){
  ...
  const wsData=[
    [`Jurnal Mengajar — ${currentTeacher.nama} — ${settings.semester_aktif} ${settings.tahun_ajaran_aktif}`],
    [],
    ['Tanggal','Kelas','Jam Ke','Tujuan Pembelajaran','Metode','Materi','Kendala','Catatan','Hadir','Sakit','Izin','Alpha','Nama Tidak Hadir']
  ];
  list.forEach(j=>{
    const attForDay=periodFilter(attendanceRecords).filter(a=>a.tanggal===j.tanggal&&a.kelas===j.kelas);
    const sakit=attForDay.filter(a=>a.status==='Sakit');
    ...
    wsData.push([
      j.tanggal, j.kelas, j.jam_ke||'', j.kd?tpLabel(j.kd,true):'',
      j.metode||'', j.materi, j.kendala||'', j.catatan||'',
      hadir, sakit.length, izin.length, alpha.length, notHadir
    ]);
  });
  const ws=XLSX.utils.aoa_to_sheet(wsData);
  ws['!cols']=[{wch:12},{wch:6},{wch:7},{wch:20},{wch:18},{wch:40},{wch:25},{wch:20},{wch:7},{wch:7},{wch:7},{wch:7},{wch:40}];
...
async function exportKeaktifanXLSX(){
  ...
  const wb=XLSX.utils.book_new();
  const wsData=[
    [`Rekap Keaktifan ${kelas} — ${settings.semester_aktif} ${settings.tahun_ajaran_aktif}`],[],
    ['No','Nama',...dates.map(fmtDateShort),'Total Skor','Rata-rata']
  ];
  list.forEach(s=>{
    ...
    dates.forEach(d=>{
      ...
      row.push(skor); if(skor>0){total+=skor;count++;}
    });
    row.push(total); row.push(count?Math.round(total/count):'');
    wsData.push(row);
  });
  const ws=XLSX.utils.aoa_to_sheet(wsData);
  ws['!cols']=[{wch:5},{wch:28},...dates.map(()=>({wch:9})),{wch:10},{wch:10}];
...
async function exportKejadianXLSX(){
  ...
  const wb=XLSX.utils.book_new();
  const wsData=[
    [`Catatan Kejadian — ${settings.semester_aktif} ${settings.tahun_ajaran_aktif}`],[],
    ['Tanggal','Nama Siswa','Kelas','Jenis','Tingkat','Deskripsi','Tindak Lanjut','Status']
  ];
  list.forEach(i=>{
    const s=studentObj(i.student_id);
    wsData.push([i.tanggal,s?s.nama:'?',s?s.kelas:'?',i.jenis,i.tingkat,i.deskripsi,i.tindak_lanjut||'',i.status]);
  });
  const ws=XLSX.utils.aoa_to_sheet(wsData);
  ws['!cols']=[{wch:12},{wch:28},{wch:6},{wch:10},{wch:8},{wch:40},{wch:30},{wch:10}];
...
async function exportGradesXLSX(){
  ...
  const typeGroups={};
  typeColumns.forEach(c=>{ if(!typeGroups[c.typeName]) typeGroups[c.typeName]=[]; typeGroups[c.typeName].push(c); });
  const wsData=[[`Rekap Nilai Matematika Kelas ${kelas} — ${settings.semester_aktif} ${settings.tahun_ajaran_aktif}`],[],];
  const h1=['No','Nama']; const h2=['',''];
  Object.entries(typeGroups).forEach(([tn,cols])=>{ h1.push(tn+` (${cols[0].bobot}%)`); for(let i=1;i<cols.length;i++) h1.push(''); cols.forEach(c=>h2.push(c.namaPenilaian)); });
  h1.push('Nilai Akhir'); h2.push('');
  ...
  list.forEach(s=>{
    ...
    typeColumns.forEach(c=>{
      if(c.isKeaktifan) row.push(avgPT[c.typeId]!==null?avgPT[c.typeId]:'');
      else if(c.isEmpty) row.push('');
      else { const g=pg.find(x=>x.student_id===s.id&&x.grade_type_id===c.typeId&&x.nama_penilaian===c.namaPenilaian); row.push(g?parseFloat(g.nilai.toFixed(1)):''); }
    });
    row.push(hasAny?parseFloat(finalScore.toFixed(1)):'');
    wsData.push(row);
  });
  const ws=XLSX.utils.aoa_to_sheet(wsData);
  ws['!cols']=[{wch:5},{wch:28},...typeColumns.map(()=>({wch:13})),{wch:12}];
